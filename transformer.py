...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., weights_dropout=True):
        ...
        self.embed_dim = embed_dim # 词向量的维度 -> 512
        self.num_heads = num_heads # The number of heads (multiheads) -> 8
        self.dropout = dropout # 默认不使用dropout
        self.head_dim = embed_dim // num_heads # 每一个head的维度（人为设定）-> 64
        assert self.head_dim * num_heads == self.embed_dim, "embed_dim must be divisible by num_heads"
        self.scaling = self.head_dim ** -0.5

        self.in_proj_weight = Parameter(torch.Tensor(3 * embed_dim, embed_dim)) # QKV so 3
        self.in_proj_bias = Parameter(torch.Tensor(3 * embed_dim)) # QKV so 3
    ...
    def _in_proj(self, input, start=0, end=None): # matrix multiplication
